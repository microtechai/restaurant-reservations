...
class RRSettings {
	...
	public function sanitize_hours( $value ) { $clean = array(); foreach ( (array) $value as $day => $times ) { $clean[ sanitize_key( $day ) ] = array( 'open' => sanitize_text_field( $times['open'] ?? '' ), 'close' => sanitize_text_field( $times['close'] ?? '' ), 'break_open' => sanitize_text_field( $times['break_open'] ?? '' ), 'break_close' => sanitize_text_field( $times['break_close'] ?? '' ) ); } return $clean; }
	...
	public function hours_field() {
		...
		foreach ( $days as $day => $label ) {
			printf( '<p><label class="rr-day-label">%s</label> <input type="time" name="rr_business_hours[%s][open]" value="%s"> <span>%s</span> <input type="time" name="rr_business_hours[%s][close]" value="%s"><br><small>%s</small> <input type="time" name="rr_business_hours[%s][break_open]" value="%s"> <span>%s</span> <input type="time" name="rr_business_hours[%s][break_close]" value="%s"></p>', esc_html( $label ), esc_attr( $day ), esc_attr( $hours[ $day ]['open'] ?? '' ), esc_html__( 'to', 'restaurant-reservations' ), esc_attr( $day ), esc_attr( $hours[ $day ]['close'] ?? '' ), esc_html__( 'Break', 'restaurant-reservations' ), esc_attr( $day ), esc_attr( $hours[ $day ]['break_open'] ?? '' ), esc_html__( 'to', 'restaurant-reservations' ), esc_attr( $day ), esc_attr( $hours[ $day ]['break_close'] ?? '' ) );
		}
	}
