...
class RRCalendar {
	/**
	 * Get the day key in English for business hours lookup.
	 *
	 * @param string $date Y-m-d
	 * @return string lowercase day name (e.g., 'sunday', 'monday')
	 */
	private function get_day_key( $date ) {
		// Array fijo de días en inglés para asegurar compatibilidad independiente del locale
		$days = array( 'sunday', 'monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday' );
		$day_index = (int) gmdate( 'w', strtotime( $date ) ); // 0=Sunday, 1=Monday, etc.
	...
	public function get_available_slots( $date, $guests ) {
		...
		$hours = $this->get_business_hours( $this->get_day_key( $date ) );
